### Model binding and validation from [gin](github.com/gin-gonic/gin)
...
### Query/Multipart/Urlencoded binding with nested struct which should be encoded by json
...
type LoginForm struct {
	User     string `form:"user" binding:"required"`
	Password string `form:"password" binding:"required"`
    Option Option `form:"option" binding:"required"`
}
...
Test it with:
```sh
$ curl -v --form user=user --form password=password --form option="{\"timeout\":10}" http://localhost:8080/login
```
